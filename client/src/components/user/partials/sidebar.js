...
const Sidebar = () => {
  ...
  function DropDowntoggle() {
    ...
    document.getElementsByClassName('fa-caret-down')[0].classList.toggle("hide");
  ...
  return (
    <>
      ...
      <div className="menu_option">
        <button onClick={menu} id="menu_btn">CATEGORIES</button>
        <button onClick={account} id="account_btn">ACCOUNT</button>
        <span id="border"></span>
      </div>
      <div id="Menu">
        <ul>
          ...
          <div className="btn-group" key={_id} >
            <li onClick={DropDowntoggle} data-toggle="dropdown" aria-expanded="true">{categoryName}<i className="fa fa-caret-down"></i><i className="fa fa-caret-up"></i></li>
            <div className="dpdownlist dropdown-menu">
               {subCategory.subCategories.data?.map((item, i) => {
                if (item.categoryId == _id) {
                  return<Link to="/" key={item._id} >{item.subCategoryName}</Link>
                }
              })}
             </div>
             </div>
      ...
      {isPreviewShown && <>
        ...
        <p>
          ...
          <a href="http://cstechsoftwares.com/"><i className="fab fa-twitter"></i></a>
          <a href="http://cstechsoftwares.com/"><i className="fab fa-instagram"></i></a>
        </p>
      </div>
