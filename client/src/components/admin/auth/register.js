...
const Register = ({ history }) => {
  ...
  const { firstName, lastName, userName, email, password, confirmPassword } = newUser;

  const onChange = (e) => setNewUser({ ...newUser, [e.target.name]: e.target.value });

  const onSubmit = e => {
    e.preventDefault();
    if (firstName === '' || lastName === '' || userName === '' || email === '' || password === '' || confirmPassword === '' || !isConfirmed)
              ...
              <div className="container-fluid">
                <div className="row">
                  <div className="col-md-6 l_inp">
                    <input type="text" name="firstName" value={firstName}  onChange={onChange} autoFocus placeholder="First name" />
                  </div>
                  ...
                  <div className="col-12">
                    <input type="password" name="password" value={password} onChange={onChange} minLength='6' placeholder="Password" />
                  </div>
                  <div className="col-12">
                    <input type="password" name="confirmPassword" value={confirmPassword} onChange={onChange} minLength='6'  placeholder="Confirm Password" />
                  </div>
                  <div className="col-12">
                    <div className="agree">
                      <label className="check"><input type="checkbox" checked={isConfirmed} onChange={handleClick} /><span className="checkmark"></span>
                      </label><span>I agree with terms and conditions</span>
                    </div>
                  </div>
                  <input type="submit" className="ripple button-base" value="Signup" />
                  <em>Already have an account? <Link to="/admin/login">Sign in </Link></em>
                </div>
              </div>
            </form>
          </div>
